...
async function ObtenerClientes() {
    ...
    const tbody = document.querySelector("#todosLosClientes tbody");
    tbody.innerHTML = "";

    clientes.forEach(item => {
        const row = document.createElement("tr");
        row.innerHTML = `
                <td class='celda-titulo data-ticket ${item.eliminado ? "data-true" : "data-false"}'>${item.nombre}</td>
                <td class='celda-titulo data-ticket'>${item.dni}</td>
                <td class='d-none d-sm-table-cell celda-titulo data-ticket'>${item.email}</td>
                <td class='d-none d-md-table-cell celda-titulo data-ticket'>${item.telefono}</td>
                <td class='d-none d-lg-table-cell celda-titulo data-ticket'>${item.observaciones}</td>
                <td class='text-center'>
                    ${item.eliminado
                        ? `<a onclick='habilitarCliente(${item.clienteID})'><i class='btn bi bi-arrow-clockwise fs-5 text-success m-2' style='text-shadow: 0 0 4px green;'></i></a>`
                        : `<a onclick='ConfirmacionEliminacionCliente(${item.clienteID})'><i class='btn bi bi-trash text-danger m-2'></i></a>
                           <a onclick='BuscarClienteparaEditar(${item.clienteID})'><i class='btn bi bi-brush text-info m-2'></i></a>`
                    }
                </td>
            `;
